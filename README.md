...
ACTIVE ROADMAP (settled 2026-05-29, strict order; see git log + memory):
  Phase 1 [wip]  Interpreter performance. Landed: per-op root sync gated on cell-membership
                 ...
                 no-cap run still exceeded ~3min; broader probe-only generated-executor disable
                 drives generated executions to zero and completes no-cap Richards at score
                 ~0.099, proving the temporary bytecode shim is the regression source while native
...
[wip] Runtime semantics
  ...
  [wip] calls, constructs, and function values
    ...
    [done] baseline-generated bytecode-executor diagnostic policy: VM config and
           octane_probe can disable the Rust-only generated executor across source entry,
           loop handoff, generated resume, direct call, and direct construct while leaving
           artifacts/telemetry/defaults intact; capped Richards reports
           baseline_generated_executions=0, and no-cap Richards completes via interpreter
           fallback (score ~0.099) under the HostBlockedX86_64 native blocker
