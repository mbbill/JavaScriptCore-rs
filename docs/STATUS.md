...
- [wip] per-kind POD-ification (retire the 9 remaining Drop fields → POD cell, cheapest-first per gc-r4.md;
  integrate serially): **BoundFunction + Promise + RegExp DONE (3/6)** (RegExp also deleted regexp_flags_text,
  recomputed canonical from bits — fixed a toString flag-order divergence); pending ArrayBuffer/Map-Set/
