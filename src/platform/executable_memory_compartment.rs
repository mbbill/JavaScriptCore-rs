...
//! function pointers. Generated-code invocation stays behind sealed helper
//! methods that keep raw executable addresses private to the platform backend.
...
pub enum ExecutableMemoryCompartmentError {
    ...
    Arm64JscStackCallRequestInvalid {
...
impl ExecutableMemoryArm64JscStackCallRequest {
    ...
    pub fn validate(self) -> Result<(), ExecutableMemoryArm64JscStackCallRequestValidationError> {
        ...
        let entry_frame = self.entry_frame.as_ptr() as usize;
        if entry_frame % Self::STACK_ALIGNMENT_BYTES != 0 {
            return Err(
                ExecutableMemoryArm64JscStackCallRequestValidationError::EntryFrameUnaligned {
                    entry_frame,
                },
            );
        }
        if entry_frame <= call_frame {
            return Err(
...
impl ExecutableMemoryCompartment {
    ...
    /// Invoke a sealed ARM64 JSC-stack entry inside an already RX compartment.
    ///
    /// This is a private normal-return-only bridge for the C++ JSC entry shape:
    /// `sp = CallFrame + sizeof(CallerFrameAndPC)`, `fp/x29 = EntryFrame`, and
    /// `lr` names the trampoline return label before generated code begins.
    /// It does not publish VM top frames, install conservative roots, or handle
    /// exception/unwind restoration.
    pub fn call_arm64_jsc_stack_entry(
        &self,
        ...
        #[cfg(all(unix, target_arch = "aarch64"))]
        {
            let mapping = self
                ...
                .ok_or(ExecutableMemoryCompartmentError::AlreadyReleased)?;
            let encoded_js_value_bits = mapping
                .call_arm64_jsc_stack_entry(request.entry_offset as usize, request)
                .map_err(|error| {
                    map_call_platform_error(
                        error,
                        ExecutableMemoryPlatformOperation::CallArm64JscStackEntry,
                    )
                })?;
            Ok(ExecutableMemoryP6CallResult {
                encoded_js_value_bits,
...
mod tests {
    ...
    fn arm64_jsc_stack_call_request(
        ...
        arm64_jsc_stack_call_request_with_entry_offset(
            12,
            entry_sp,
            call_frame,
            entry_frame,
    ...
    fn arm64_jsc_stack_call_request_with_entry_offset(
        entry_offset: u32,
        entry_sp: usize,
        call_frame: usize,
        entry_frame: usize,
    ...
    fn arm64_jsc_stack_call_request_validates_vm_entry_callee_save_buffer_shape() {
        let count_mismatch = arm64_jsc_stack_call_request_with_callee_save_shape(
            12, 0x2010, 0x2000, 0x3000, 0x2f70, 17, 144,
        );
        assert_eq!(
        ...
        let size_mismatch = arm64_jsc_stack_call_request_with_callee_save_shape(
            12, 0x2010, 0x2000, 0x3000, 0x2f70, 18, 136,
        );
        assert_eq!(
    ...
    #[cfg(unix)]
    fn valid_arm64_jsc_stack_call_request_at_offset(
        entry_offset: u32,
    ) -> ExecutableMemoryArm64JscStackCallRequest {
        arm64_jsc_stack_call_request_with_entry_offset(entry_offset, 0x2010, 0x2000, 0x3000, 0x2f70)
    }

    #[cfg(unix)]
    #[test]
    fn arm64_jsc_stack_call_rejects_writable_mapping_before_platform_dispatch() {
        ...
        assert_eq!(
            compartment.call_arm64_jsc_stack_entry(valid_arm64_jsc_stack_call_request_at_offset(0)),
            Err(ExecutableMemoryCompartmentError::WrongProtection {
                expected: ExecutableMemoryProtection::Executable,
                actual: ExecutableMemoryProtection::Writable,
            })
    ...
    #[cfg(unix)]
    #[test]
    fn arm64_jsc_stack_call_rejects_released_mapping_before_platform_dispatch() {
        let bytes = [0xc0, 0x03, 0x5f, 0xd6];
        let mut compartment =
            ExecutableMemoryCompartment::allocate(test_request(bytes.len() as u32)).unwrap();
        compartment
    ...
    #[cfg(unix)]
    #[test]
    fn arm64_jsc_stack_call_rejects_entry_offset_out_of_range() {
        let bytes = [0xc0, 0x03, 0x5f, 0xd6];
        let mut compartment =
            ExecutableMemoryCompartment::allocate(test_request(bytes.len() as u32)).unwrap();
        compartment
        ...
        assert_eq!(
            compartment.call_arm64_jsc_stack_entry(valid_arm64_jsc_stack_call_request_at_offset(
                bytes.len() as u32
            )),
    ...
    #[cfg(unix)]
    #[test]
    fn arm64_jsc_stack_call_rejects_invalid_request_before_platform_dispatch() {
        let bytes = [0xc0, 0x03, 0x5f, 0xd6];
        let mut compartment =
            ExecutableMemoryCompartment::allocate(test_request(bytes.len() as u32)).unwrap();
        compartment
        ...
        let request =
            arm64_jsc_stack_call_request_with_entry_offset(0, 0x2018, 0x2000, 0x3000, 0x2f70);
        assert_eq!(
            compartment.call_arm64_jsc_stack_entry(request),
            Err(
                ExecutableMemoryCompartmentError::Arm64JscStackCallRequestInvalid {
                    reason:
                        ExecutableMemoryArm64JscStackCallRequestValidationError::EntryStackPointerMismatch {
                            expected_entry_sp: 0x2010,
                            actual_entry_sp: 0x2018,
    ...
    #[cfg(all(unix, not(target_arch = "aarch64")))]
    #[test]
    fn arm64_jsc_stack_call_is_unsupported_after_safe_checks_off_aarch64() {
        let bytes = [0xc0, 0x03, 0x5f, 0xd6];
        let mut compartment =
            ExecutableMemoryCompartment::allocate(test_request(bytes.len() as u32)).unwrap();
        compartment
    ...
    #[cfg(all(unix, target_arch = "aarch64"))]
    #[repr(align(16))]
    struct AlignedArm64JscStack([u8; 768]);

    #[cfg(all(unix, target_arch = "aarch64"))]
    fn arm64_jsc_stack_request_from_aligned_stack(
        entry_offset: u32,
        stack: &mut AlignedArm64JscStack,
    ) -> (ExecutableMemoryArm64JscStackCallRequest, usize, usize) {
        let base = stack.0.as_mut_ptr() as usize;
        let call_frame = base + 0x100;
        let entry_sp =
            call_frame + ExecutableMemoryArm64JscStackCallRequest::CALLER_FRAME_AND_PC_BYTES;
        let entry_frame = base + 0x200;
        let callee_save_buffer =
            entry_frame - ExecutableMemoryArm64JscStackCallRequest::VM_CALLEE_SAVE_BUFFER_BYTES;
        (
            arm64_jsc_stack_call_request_with_entry_offset(
                entry_offset,
                entry_sp,
    ...
    #[cfg(all(unix, target_arch = "aarch64"))]
    fn call_arm64_jsc_stack_test_stub(
        bytes: &[u8],
        request: ExecutableMemoryArm64JscStackCallRequest,
    ) -> ExecutableMemoryP6CallResult {
        let mut compartment =
        ...
        compartment
            ...
            .unwrap();
        compartment.protect_executable().unwrap();
        compartment.call_arm64_jsc_stack_entry(request).unwrap()
    }

    #[cfg(all(unix, target_arch = "aarch64"))]
    #[test]
    fn arm64_jsc_stack_trampoline_enters_rx_stub_with_requested_sp() {
        let mut stack = AlignedArm64JscStack([0; 768]);
        let (request, entry_sp, _) = arm64_jsc_stack_request_from_aligned_stack(0, &mut stack);
        let result = call_arm64_jsc_stack_test_stub(
            &[
        ...
        assert_eq!(
            result,
            ExecutableMemoryP6CallResult {
                encoded_js_value_bits: entry_sp as u64,
            }
        );
    }
    ...
    #[cfg(all(unix, target_arch = "aarch64"))]
    #[test]
    fn arm64_jsc_stack_trampoline_enters_rx_stub_with_entry_frame_as_fp() {
        let mut stack = AlignedArm64JscStack([0; 768]);
        let (request, _, entry_frame) = arm64_jsc_stack_request_from_aligned_stack(0, &mut stack);
        let result = call_arm64_jsc_stack_test_stub(
            &[
        ...
        assert_eq!(
            result,
            ExecutableMemoryP6CallResult {
                encoded_js_value_bits: entry_frame as u64,
            }
        );
    }
    ...
    #[cfg(all(unix, target_arch = "aarch64"))]
    #[test]
    fn arm64_jsc_stack_trampoline_supports_jsc_prologue_frame_anchor() {
        let mut stack = AlignedArm64JscStack([0; 768]);
        let (request, _, entry_frame) = arm64_jsc_stack_request_from_aligned_stack(0, &mut stack);
        let result = call_arm64_jsc_stack_test_stub(
            &[
                ...
                0xa0, 0x03, 0x40, 0xf9, // ldr x0, [x29]
