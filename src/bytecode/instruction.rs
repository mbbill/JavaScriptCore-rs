...
impl PackedInstructionStream {
    ...
    /// POST-GENERATION ENCODER PASS (ratified serial decision, ordinal
    /// declarations -> raw packed bytes). Called once by
    /// `BytecodeGenerator::finish()` (`bytecode/generator.rs`) right after a
    /// function/program's `declarations` are frozen: if every declared
    /// instruction is packed-expressible in the currently-supported opcode
    /// family (see [`try_encode_declarations_as_raw`]), the real packed byte
    /// layout is attached to `raw` so `dfg::parser` (which reads ONLY
    /// `raw_bytes()`) can see real bytecompiler output instead of hand-built
    /// `InstructionStreamWriter` test fixtures. Otherwise `raw` stays
    /// `Unencoded`, exactly as before this pass existed — a strict addition,
    /// never a behavior change for the `declarations` domain the interpreter
    /// still dispatches off (see the `instruction_stream` module doc: the
    /// cursor already prefers the ordinal `declarations`/`typed_placeholder`
    /// forms over raw bytes, so attaching `raw` here is inert for execution).
    ///
    /// TRANSITIONAL: this is scaffolding for the divergence
    /// `instruction_stream.rs`'s module doc tracks retiring (Prime Focus #2)
    /// — the ORDINAL `declarations` form is JSC-divergent; the packed `raw`
    /// form produced here is the faithful one. A real `BytecodeGenerator`
    /// emits directly into the packed stream as it walks the AST; there is no
    /// C++ "encode after the fact" step. Per-family dispatch cutover (making
    /// the interpreter itself consume `raw` instead of `declarations`) is a
    /// separate, later, real-bench-gated batch (see
    /// `jit/arm64_baseline/function_emitter.rs` divergence #1 for the
    /// matching JIT-side note).
    pub fn with_raw_encoded_from_declarations(mut self) -> Self {
        if let Some(bytes) = try_encode_declarations_as_raw(&self.declarations) {
            self.raw = PackedByteStorage::Owned(bytes);
        }
        self
    }
...
/// Try to encode one CodeBlock's frozen `declarations` into JSC's real packed
/// byte layout. See [`PackedInstructionStream::with_raw_encoded_from_declarations`]
/// for the call site and the transitional-scaffolding rationale.
///
/// SUPPORTED FAMILY (G4-Unit-1, ratified): `op_enter` (synthesized, see
/// below), `op_mov`, `op_ret`, `op_add`/`op_sub`/`op_mul`. `None` — decline
/// the WHOLE function, never a partial stream — the moment any declared
/// instruction falls outside that family, mirroring
/// `dfg::parser::capability_level`'s own all-or-nothing admission: a `None`
/// here must always mean "this body cannot be raw-encoded yet", not
/// "encoding stopped partway".
fn try_encode_declarations_as_raw(declarations: &[InstructionDeclaration]) -> Option<Vec<u8>> {
    // A well-formed CodeBlock always declares at least one instruction (every
    // generated body ends with a Return, `bytecompiler/mod.rs` `emit_return`
    // call sites). Declining outright on an empty list rather than emitting
    // an `op_enter`-only stream keeps this pass a no-op for anything that
    // was not really generated by the bytecompiler (e.g. a stream some other
    // constructor left empty), matching the pre-existing
    // `NoRawPackedInstructionStream` decline for that case exactly.
    if declarations.is_empty() {
        return None;
    }
    let mut writer = raw_stream::InstructionStreamWriter::new();

    // `BytecodeGenerator::emitEnter()` (BytecodeGenerator.cpp:1439-1449) runs
    // FIRST, unconditionally, in every one of the generator's four
    // constructors (:404 program, :539 function, :999 eval, :1095 module):
    // `op_enter` carries NO operands (`BytecodeList.rb:1180`), and its
    // semantics (callee-local zero-fill) are supplied by its OWN LLInt
    // handler at RUNTIME (`llint_op_enter`), not computed by the generator
    // from the AST. The Rust `declarations` domain has no analog instruction
    // — DIVERGENCE #1, already documented at
    // `jit/arm64_baseline/function_emitter.rs:37-43`: the interpreter
    // instead zero-fills callee locals at frame push
    // (`vm/jsstack.rs:1157-1165`). Synthesizing the leading `op_enter` byte
    // here is therefore not derived/invented data — it is a structural
    // constant true of EVERY real JSC CodeBlock regardless of AST content,
    // reconstructed for the one consumer (DFG) that requires the real byte
    // layout the `declarations` domain does not carry.
    writer.emit(raw_stream::opcode_id::ENTER, &[]);

    // Ordinal count of add/sub/mul seen so far. This equals
    // `code_block::derive_binary_arith_profiles`'s per-CodeBlock slot index
    // for the SAME instruction as long as every earlier
    // binary-arith-profiled opcode (`FOR_EACH_OPCODE_WITH_BINARY_ARITH_PROFILE`,
    // Opcode.h:158-167) in this function is also add/sub/mul — guaranteed
    // here because any OTHER member of that set (div/bitand/bitor/bitxor/
    // lshift/rshift) is outside the family below and bails the WHOLE
    // function before a later add/sub/mul could be reached.
    let mut arith_profile_index: u32 = 0;

    for declaration in declarations {
        let core = CoreOpcode::from_opcode(declaration.opcode)?;
        match core {
            CoreOpcode::Move => {
                // `op_mov` args `{ dst, src }` (BytecodeList.rb:1248-1252);
                // `emit_move` (bytecompiler/mod.rs) declares operands in
                // that exact order.
                let [dst, src] = declared_registers(&declaration.operands)?;
                writer.emit(
                    raw_stream::opcode_id::MOV,
                    &[
                        raw_stream::OperandValue::VirtualRegister(dst.raw()),
                        raw_stream::OperandValue::VirtualRegister(src.raw()),
                    ],
                );
            }
            CoreOpcode::Return => {
                // `op_ret` args `{ value }` (BytecodeList.rb:1040-1043).
                // `ReturnDerived` (2 operands: value + this) has no raw
                // opcode of its own in this family and falls to the wildcard
                // arm below.
                let [value] = declared_registers(&declaration.operands)?;
                writer.emit(
                    raw_stream::opcode_id::RET,
                    &[raw_stream::OperandValue::VirtualRegister(value.raw())],
                );
            }
            CoreOpcode::AddInt32 | CoreOpcode::SubInt32 | CoreOpcode::MulInt32 => {
                // `op_add`/`op_sub`/`op_mul` args `{ dst, lhs, rhs,
                // profileIndex, operandTypes }` (BytecodeList.rb:1276-1292);
                // `emit_binary_opcode` (bytecompiler/mod.rs) declares
                // `[dst, left, right]` in that order.
                let [dst, lhs, rhs] = declared_registers(&declaration.operands)?;
                let id = match core {
                    CoreOpcode::AddInt32 => raw_stream::opcode_id::ADD,
                    CoreOpcode::SubInt32 => raw_stream::opcode_id::SUB,
                    _ => raw_stream::opcode_id::MUL,
                };
                let profile_index = arith_profile_index;
                arith_profile_index = arith_profile_index.saturating_add(1);
                writer.emit(
                    id,
                    &[
                        raw_stream::OperandValue::VirtualRegister(dst.raw()),
                        raw_stream::OperandValue::VirtualRegister(lhs.raw()),
                        raw_stream::OperandValue::VirtualRegister(rhs.raw()),
                        raw_stream::OperandValue::UnsignedImmediate(profile_index),
                        // No `ResultType` tracking exists in the Rust
                        // bytecompiler yet (`emit_binary_opcode` carries only
                        // register operands): emit JSC's own DEFAULT —
                        // `OperandTypes(unknownType(), unknownType())`
                        // (`ResultType.h:246-249`) — rather than invent
                        // narrower types the generator never computed.
                        raw_stream::OperandValue::OperandTypes(
                            raw_stream::UNKNOWN_OPERAND_TYPES_BITS,
                        ),
                    ],
                );
            }
            // Every other CoreOpcode (including the other
            // binary-arith-profiled ops above) is outside the supported
            // family: decline the whole function, never a partial stream.
            _ => return None,
        }
    }

    Some(writer.finalize().bytes().to_vec())
}

/// Extract exactly `N` plain register operands, or decline (`None`). Every
/// declaration in the supported family above carries only valid register
/// operands (see each arm's `BytecodeList.rb` citation); any other operand
/// kind, arity, or an invalid register is not packed by this encoder.
fn declared_registers<const N: usize>(operands: &[Operand]) -> Option<[VirtualRegister; N]> {
    if operands.len() != N {
        return None;
    }
    let mut registers = [VirtualRegister::INVALID; N];
    for (slot, operand) in registers.iter_mut().zip(operands) {
        let register = operand.as_register()?;
