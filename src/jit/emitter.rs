...
// later full ARM64 baseline lane. C++ AssemblyHelpers::emitFunctionPrologue()
// also calls tagReturnAddress(); on ARM64E JSC that signs lr as part of its
// PtrTag/entry machinery. This raw Rust C-ABI entry is invoked as a function
// pointer over anonymous RX memory, outside that JSC tagged-entry contract, so
// it intentionally emits no PAC instruction until the full ARM64E/JSC entry
// model is ported.
...
struct P6X86_64SemanticEncodedSelection {
    ...
    js_call_owner_post_call_stubs: Vec<P9X86_64BaselineOwnerCallPostCallStubRecord>,
    js_call_owner_post_call_reentry_stubs: Vec<P9X86_64BaselineOwnerCallPostCallReentryStubRecord>,
